...
{
    public class WebH5PayController : ApiController
    {
        [HttpPost]
        public HttpResponseMessage WebH5PayRequest([FromBody] RequestData obj)
        {
            SortedDictionary<string, string> requestInfo = new SortedDictionary<string, string> {
                { "service", "pay.qrcode.waph5pay" },
                { "mch_id", "84931015812A00N" },
                { "org_code", "50264239" },
                { "nonce_str", "123456789" },
                { "pay_type", "alipay" },//交易类型    wxpay、alipay、unionpay
                { "out_trade_no", "20190413111836850" },
                { "total_fee", "10" },
                { "body", "京东生鲜" },
                { "term_no", "800056" },//终端设备号   unionpay时必须8位
                { "device_info", "800056" },
                ...
                ///非必传字段
                ///</summary>
                { "buyer_logon_id", "13517198483" }, //买家支付宝账号       buyer_logon_id和buyer_id不能同时为空
                { "buyer_id", "" },
                { "version", "V1.0.1" },
                { "charset", "utf-8" },
                { "sign_type", "RSA256" },
