...
{
    public class QueryOrderController : ApiController
    {
        [HttpPost]
        public HttpResponseMessage QueryOrderRequest([FromBody] RequestData obj)
        {
            SortedDictionary<string, string> requestInfo = new SortedDictionary<string, string> {
                { "service", "pay.qrcode.pay.qrcode.query" },
                { "mch_id", "84931015812A00N" },
                { "org_code", "50264239" },
                { "nonce_str", "123456789" },
                { "out_trade_no", "20190415210736700" },
                ///<summary>
                ///非必传字段
                ///</summary>
                { "version", "V1.0.1" },
