    ...
    public class AppPayController : ApiController
    {
        ...
        public HttpResponseMessage AppPayRequest([FromBody] RequestData obj)
        {
            SortedDictionary<string, string> requestInfo = new SortedDictionary<string, string>
            {
                ...
                ///非必传字段
                ///</summary>
                { "user_id", "" }, //银联二维码用户标识     unionpay时必须传
                { "version", "V1.0.1" },
                { "charset", "utf-8" },
                { "sign_type", "RSA256" },
                { "remark", "备注" },
