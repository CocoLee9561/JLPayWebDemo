...
namespace JlpayWebApi.Controllers
{
    ...
    /// 条形码支付异步demo
    /// </summary>
    public class MicroPayAsynController : ApiController
    {
        [HttpPost]
        public HttpResponseMessage MicroPayAsynRequest([FromBody] RequestData obj)
        {
            SortedDictionary<string, string> requestInfo = new SortedDictionary<string, string> {
                { "service", "pay.qrcode.micropayasyn" },
                { "mch_id", "84931015812A00N" },
                { "org_code", "50264239" },
                { "nonce_str", "123456789" },
                { "pay_type", "wxpay" },//交易类型    wxpay、alipay、unionpay
                { "out_trade_no", "20190415210736800" },
                { "total_fee", "10" },
                { "body", "京东生鲜" },
                { "term_no", "800056" },
                { "device_info", "800056" },
