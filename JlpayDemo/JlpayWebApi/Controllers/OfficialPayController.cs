...
{
    public class OfficialPayController : ApiController
    {
        [HttpPost]
        public HttpResponseMessage OfficialPayRequest([FromBody] RequestData obj)
        {
            SortedDictionary<string, string> requestInfo = new SortedDictionary<string, string> {
                { "service", "pay.qrcode.officialpay" },
                { "mch_id", "84931015812A00N" },
                { "org_code", "50264239" },
                { "nonce_str", "123456789" },
                { "pay_type", "alipay" },//交易类型    wxpay、alipay、unionpay
                { "out_trade_no", "20190413111836309" },
                { "total_fee", "10" },
                { "body", "京东生鲜" },
                ...
                ///非必传字段
                ///</summary>
                { "sub_appid", "" },//银联二维码用户标识     unionpay时必须传
                { "version", "V1.0.1" },
                { "charset", "utf-8" },
                { "sign_type", "RSA256" },
                { "remark", "备注" },
